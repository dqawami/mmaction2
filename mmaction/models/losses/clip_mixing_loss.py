...
class ClipMixingLoss(BaseWeightedLoss):
    ...
    def _forward(self, logits, norm_embd, scale=None):
        if self.mode == 'embd':
            norm_embd = norm_embd.view(norm_embd.size(0) // self.num_clips, self.num_clips, -1)
            ...
            logits = scale * logits.view(logits.size(0) // self.num_clips, self.num_clips, -1)
